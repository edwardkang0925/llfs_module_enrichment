...
# this script was written for LLFS module enrichment downstream analysis. Comparing power between two studies based on the enrichment of module GO terms.
setwd("/Users/test/projects/llfs_module_enrichment")

# Things to change
STUDIES = c('twas', 'cma')
MODULEFILEROOT = "./outputs/goByModules/sig4genes"
BACKGROUNDROOT = "./outputs/goByModules/background"
SUMMARYROOT = "./outputs/goByModules/GO_summaries"
REPORTROOT = "./outputs/goByModules/GO_reports"

fdrThrs = c(0.05, 0.1)

# WebGestalt parameters
METHOD="ORA" # ORA | GSEA | NTA
DATABASES=c("geneontology_Biological_Process") # "geneontology_Biological_Process "geneontology_Molecular_Function" # see options with listGeneSet()
GENE_ID="genesymbol" # see options with listIdType()

for (fdrThr in fdrThrs){
  for (STUDY in STUDIES) {
    TRAITS = list.dirs(file.path(BACKGROUNDROOT, STUDY), full.names=FALSE, recursive=FALSE)
    for (TRAIT in TRAITS) {
      for (DATABASE in DATABASES) {
        INPUT_PATH = file.path(MODULEFILEROOT, STUDY, TRAIT)
          ...
          for (file_name in list.files(INPUT_PATH)) {
            ...
            if (!is.null(enrich_df)) {
              ...
              if (nrow(apGO_full) > 0) {
                ...
                write.csv(apGO_full,file.path(SUMMARIES_PATH,paste0(name,"_",fdrThr,".csv")),row.names = FALSE)
              } else {
                print("NO SIGNIFICANT OVERLAPS")
                write.csv(NULL,file.path(SUMMARIES_PATH,paste0(name,"_",fdrThr,".csv")),row.names = FALSE)
              }
            } else {
              print("NO SIGNIFICANT OVERLAPS")
              write.csv(NULL,file.path(SUMMARIES_PATH,paste0(name,"_",fdrThr,".csv")),row.names = FALSE)
